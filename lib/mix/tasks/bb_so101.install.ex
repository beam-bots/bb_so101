...
if Code.ensure_loaded?(Igniter) do
  defmodule Mix.Tasks.BbSo101.Install do
    ...
    `shoulder_lift` and `elbow_flex` carry a transmission offset equal to the
    midpoint of their range, because their URDF frame and their motor's differ
    by that much. The gripper carries none: `mix bb_so101.calibrate` puts its
    zero 10 degrees above the closed stop, which is exactly where the URDF's
    -10 degree lower limit wants it.

    An arm last calibrated before that anchoring landed still needs the old
    45 degree offset. Recalibrate the gripper before taking a robot module
    generated by this version, or it will read 45 degrees out the other way.
    """
